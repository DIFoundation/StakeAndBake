...
export default function StakeForm() {
  ...
  const [selectedToken] = useState("XFI");
  ...
  const handleStake = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const stakingContractABI = ["function stake(uint256 amount) external"];
      ...
      // Convert amount to Wei and send the transaction
          ...
          <form className="flex flex-col" onSubmit={handleStake}>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between px-4 py-3 rounded-lg bg-[#141414] text-white cursor-pointer">
                <span
                  className={selectedToken ? "text-white" : "text-gray-500"}
                >
